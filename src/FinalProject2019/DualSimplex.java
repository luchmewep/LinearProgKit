...
public class DualSimplex {
	...
	public DualSimplex(Problem problem) {
		setProblem(problem);
		setTempEquation();
		setTempConstraints();
	}
	...
	public void setTempEquation() {
		if(p.getNumVariables()>0) {
			tempEquation = new ArrayList<String>(Arrays.asList(p.getEquation()));
			tempEquation.add(0, "");
	...
	private void setTempFooter() {
		tempFooter = new ArrayList<String>();
		for (int i = 0; i < tempEquation.size(); i++) {
			tempFooter.add("");
		}
	}
	private void setInitialTableau() {
		tempTableau = new ArrayList<ArrayList<String>>();
